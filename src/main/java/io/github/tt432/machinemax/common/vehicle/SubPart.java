...
import io.github.tt432.machinemax.util.mechanic.ArmorUtil;
import io.github.tt432.machinemax.util.mechanic.DynamicUtil;
import io.github.tt432.machinemax.util.mechanic.MassUtil;
...
//        MachineMax.LOGGER.debug("onEnded: {} {}", pcoA.name, pcoB.name);
    ...
    public void onProcessed(PhysicsCollisionObject pcoA, @NotNull PhysicsCollisionObject pcoB, long manifoldPointId) {
        ...
        if (other.getCollisionGroup() == VehicleManager.COLLISION_GROUP_BLOCK) {
            ...
            //TODO:配置文件开关冲撞可破坏方块
            if (blockState.getDestroySpeed(part.level, other.blockPos) > 0) {//碰撞的方块可破坏时
                //计算碰撞法线方向上的速度(考虑冲量影响)
