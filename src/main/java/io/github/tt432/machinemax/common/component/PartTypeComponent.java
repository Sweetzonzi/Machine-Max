...
public record PartTypeComponent(PartType partType) {
    ...
    private int getPartTypeId() {
        return PartType.PART_TYPE.getRegistry().get().getId(partType);
    }
    ...
    private static PartTypeComponent getPartType(String registryKey) {
        return new PartTypeComponent(PartType.PART_TYPE.getRegistry().get().get(ResourceLocation.parse(registryKey)));
    }

    private static PartTypeComponent getPartTypeById(int partTypeId) {
        return new PartTypeComponent(PartType.PART_TYPE.getRegistry().get().byId(partTypeId));
    }
}
