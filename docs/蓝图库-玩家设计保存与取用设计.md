...
- **`meta` 只进 `CODEC`，不进 `STREAM_CODEC`**：服务端从存档恢复载具时不关心元信息，`VehicleData` 本身跨网络时不携带；需要时由网络载荷以**独立字段**携带——抄录走 `VehicleDataSavedPayload`，取出走 `BlueprintExtractLocalPayload`（见 §6.2 / §6.3）。
...
`VehicleData(VehicleCore)` 会带上世界坐标与速度。所有载具物品（收纳 / 蓝图）写入前统一做**坐标归一化**（`rebased()`）；蓝图额外把血量比例归满（`normalized()`）：

| 字段                                    | 归一化                          |
| ------------------------------------- | ---------------------------- |
| `pos`                                 | 归零                           |
| `SubPartData.posRotVelVel.position`   | 整体平移 `-pos`（保持与 `pos` 的相对关系） |
| `SubPartData.posRotVelVel.linearVel` / `angularVel` | 归零            |
| `hpRatio`（仅蓝图 `normalized()`）      | 置 1                          |

> **骨架语义不靠归一化**：进度、材料进度、耐久比例与连接器 / 子系统数据由放置时的 `readAdditionalData=false` 直接忽略，存的时候无需重复清零。

> **子零件坐标必须随 `pos` 一起平移**：`SubPartData.posRotVelVel.position` 记录的是绝对世界坐标，
> 而 `VehicleCore.setPos` 按「目标位置 − 当前 `pos`」的相对量移动物理体。只归零 `pos` 而不平移子零件，
> 重建后的相对位移会叠加抄录点坐标，载具被推到错误位置（表现为服务端无碰撞、客户端不可见）。
> 平移后 `pos` 与子零件坐标都以原点为基准，相对关系不变，`rebased()` 亦因此幂等。
...
- `BlueprintProblem(Kind kind, String detail)`，`Kind`：`MISSING_PART_TYPE` / `MISSING_VARIANT` / `INVALID_VALUE`。
- 方法**不抛异常**，空列表表示通过。
- 调用点在**构造 / 解析之后**，不塞进 `Codec`：`Codec` 无 `Level`，且 `registryKey` 只是 `ResourceLocation`，解码阶段不会失败。
...
sequenceDiagram
  ...
  S-->>U: VehicleDataSavedPayload（VehicleData + 独立 meta）
  U->>U: 写 saved_blueprints/<新生成 uuid>.json（展示名存 name）
...
- 新增 `public VehicleData rebased()`：坐标归零 + 子零件坐标平移 + 速度清零，**所有载具物品通用**（收纳与蓝图）。
- 新增 `public VehicleData normalized()`：`rebased()` + 血量比例归满，蓝图抄录 / 取出使用。
...
| 写入 | 抄录时按 `VehicleData.CODEC` 写入；`存入库` 由服务端读取手中 `vehicle_blueprint` 的 `VEHICLE_DATA` 后回传 `VehicleDataSavedPayload`（含独立 `meta`），客户端再写文件 |
| 重名 | 文件名与展示名解耦：**每次写入生成新的随机 UUID** 作为文件名（`<uuid>.json`，与 `VehicleData.uuid` 无关），展示名取自 JSON 内 `name`；**不再静默覆盖** |
...
- `BlueprintLibraryError` 字段：`fileName`、`reason`（枚举：`PARSE_ERROR` / `MISSING_PARTS`）、`detail`（缺失的零件 / 变体 id 列表，或异常消息）。
...
**注意**：文件名不再使用 `makeSafeFileName` 的结果（改为客户端写入时**每次生成新的随机 UUID** 作为文件名），因此服务端「已保存到 `saved_blueprints/<fileName>`」的提示需改为「已保存到蓝图库」，避免提示名与实际写入名不一致。

**元数据写入**：抄录时由服务端在 `VehicleNamingMenu.saveVehicleWithName` 中填充 `author`（玩家名）、`created_at`（当前时间）、`description`（`VehicleNamingScreen` 新增的描述输入框）；`name` 沿用现有命名流程。**`meta` 不并入 `VehicleData` 的网络编码**，而是在 `VehicleDataSavedPayload` 中作为**独立字段**随包发送，客户端写文件时再合并进 JSON。
...
| `BlueprintExtractResultPayload`  | S→C | `success`、`cost`、`reason`（枚举：结构非法 / 零件缺失 / 余额不足 / 未知蓝图）、`missingPartTypes`（缺失零件 id 列表，仅「零件缺失」时非空） |
...
- **`meta` 一律以独立字段随载荷传输，不并入 `VehicleData.STREAM_CODEC`**：`BlueprintMeta` 自带 `STREAM_CODEC`，被 `BlueprintExtractLocalPayload`（取出）与 `VehicleDataSavedPayload`（抄录 / 存入库，需为现有载荷**新增 `meta` 字段**）使用；`VehicleData.STREAM_CODEC` 不调用它。因此客户端手中 `vehicle_blueprint` 的物品组件（`VEHICLE_DATA` 的网络同步走 `VehicleData.STREAM_CODEC`）**不含 meta**，`存入库` 必须由服务端读取组件后回传，不能由客户端直接读组件。
- `blueprintId` 用 `ResourceLocation.STREAM_CODEC`；`reason` 用枚举 `StreamCodec`。
- 全部走主线程处理器（对齐 `network/AGENTS.md` 的「严禁物理线程载荷处理器」）。
- 玩家库请求体积可能较大（`VehicleData` 全量）；数据量约束由原版网络包尺寸限制承担，暂不额外设上限，也不做解码期规模校验。
...
| 分区    | 数据来源                          | 可用操作               |
| ----- | ----------------------------- | ------------------ |
| 我的设计  | `valid` + `broken` 两个列表 | 预览、取出、**存入库**（**仅对 `LOCAL` 蓝图有效**：手中物品需带 `VEHICLE_DATA` 组件；`PACK` 蓝图只有 `VEHICLE_BLUEPRINT_PATH`，该操作禁用）、删除（本地文件）、重命名 |
| 内容包蓝图 | `MMDynamicRes.BLUEPRINTS`     | 预览、取出              |

> **`存入库` 走服务端往返**：客户端手中物品的 `VEHICLE_DATA` 组件不含 `meta`（见 §6.3），不能直接读组件写文件。需新增一条 C→S 请求包，服务端校验物品并读取组件后回传 `VehicleDataSavedPayload`（含独立 `meta`），客户端再写文件。
...
**无配方零件的兜底**：[Part.assemble](file:///d:/Files/Project_MinecraftMods/Machine-Max/src/main/java/io/github/sweetzonzi/machine_max/common/mech/vehicle/Part.java#L766-L769) 中 `recipe == null && getAssemblingProgress() < 1f` 的慢速兜底分支（每 `progress` 推进 0.01）必须保留。不要删掉该分支（否则无配方零件永远无法装配），也不要只加空判断 / 去掉 `recipe == null` 限定（否则「非零件配方」会走无材料兜底，等于免费组装）。门禁侧对 `getRecipe() == null` 同样按第 1 条放行。
...
> 本节是**唯一权威**的校验清单，§4.3 只定义 `validate(Level)` 的接口与错误模型。载具规模（零件数 / 刚体数 / 连接数）暂不设上限，数据量约束由原版网络包尺寸限制承担。
...
6. **请求体积**：`VehicleData` 全量传输；数据量约束由原版网络包尺寸限制承担，暂不额外设上限。
7. **`rebuildAvailableRecipes`** **的 TODO**：`BlueprintAttachment.java` 的「蓝图库检查」指的是**计划中的蓝图收纳道具**（统一存放玩家的制造蓝图，避免背包被蓝图塞满），本质是扫描该道具内保存的附件信息，与本设计的「客户端蓝图库」无关，保留待实现。
