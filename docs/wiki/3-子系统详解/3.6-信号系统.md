...
任何子系统都可以成为信号发送者或接收者，甚至同时兼任两者——例如车辆控制器既接收座舱的 `move_input`，又向发动机发送 `engine_control`。
...
- **`signal_targets`**：定义本端口接收哪些信号以及转发给谁。目标可以是本零件内的子系统名、连接点名、`"local"`（所属 Part）或 `"global"`（所属装配体）。
...
以 AE86 底盘为例，座舱座椅位于一个独立零件中，控制器和发动机位于底盘零件中。座舱通过连接点的信号端口将 `move_input` 信号转发给底盘上的控制器：
...
flowchart LR
    ...
    seat -->|"move_input"| connector_a
    connector_a -->|"跨零件转发"| connector_b
    connector_b -->|"move_input"| ctrl
    ctrl -->|"engine_control"| engine
    ctrl -->|"wheel_control"| wheel
    engine -->|"engine_speed"| ctrl
```

在底盘零件的 JSON 中，控制器通过 `control_outputs` 指定统一控制输出频道，各受控子系统通过握手机制自动在对应频道上注册：
...
对于 UGC 作者来说，这意味着通常**不需要手动在控制器的** **`control_outputs`** **中逐个列出所有受控子系统名称**——只要发动机的 `speed_outputs` 指向了控制器，控制器就会自动注册它并下发指令。变速箱和轮胎同理。
...
"connector.machine_max.driver_seat": {
  ...
  "signal_targets": {
    ...
    "regular_input": ["subsystem.machine_max.car_controller"]
  }
}
```

这里 `signal_targets` 表示：当此连接点的信号端口收到 `move_input` 频道的信号时，转发给本零件内的 `car_controller` 子系统。由于此连接点与座椅零件上的对应连接点对接，座椅发出的信号会沿物理连接传播到达这里。
...
| 频道名               | 方向           | 信号类型                 | 含义                    |
| ----------------- | ------------ | -------------------- | --------------------- |
| `move_input`    | 座舱 → 控制器     | `MoveInputSignal`    | 玩家移动输入（WASD、空格、Shift） |
| `regular_input` | 座舱 → 控制器     | `RegularInputSignal` | 常规按键（离合器、升降挡、手刹、灯光等）  |
| `aim_input`     | 座舱 → 控制器/炮塔  | `MoveInputSignal`    | 瞄准输入                  |
...
1. **座舱是否正确接入控制链**：检查座椅子系统的 `move_outputs` 是否指向了正确的连接点，且该连接点的 `signal_targets` 是否将 `move_input` 转发给了控制器。
...
- 如果照明由信号控制（例如通过 `regular_input` 的 `TOGGLE_LIGHT` 按键），确认信号路径是否通畅；
...
如果希望在 Molang 动画中查询信号值（例如根据车速切换车轮旋转速度、根据挡位显示仪表板），不要在每帧重新解析子系统的信号频道，而是将信号输出配置为同时发送到 `"global"` 目标。控制器示例：

```json
"speed_outputs": {
  "vehicle_speed": ["local", "global"]
},
"throttle_outputs": {
  "throttle": ["local", "global"]
},
"brake_outputs": {
  "brake": ["local", "global"]
}
...
在 Molang 表达式中通过 `global.get('vehicle_speed')`、`global.get('throttle')`、`global.get('brake')` 等查询对应信号值即可获得当前车速、油门、刹车等数据（`local.get(...)` 读取的是本 Part 的存储）。
