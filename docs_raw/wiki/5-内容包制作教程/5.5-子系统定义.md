...
| `control_inputs`        | 字符串数组    | 否  | `["engine_control", "move_control"]` | 油门控制信号的接收频道名，优先级从高到低。系统按顺序查找第一个有值的信号作为油门输入                  |
...
```json
{
  ...
  "control_inputs": ["engine_control", "move_control"],
  "sounds": {
...
| `control_inputs`        | 字符串数组    | 否  | `["motor_control", "engine_control", "move_control"]` | 油门控制信号的接收频道名，优先级从高到低                                      |
...
```json
{
  ...
  "control_inputs": ["motor_control", "move_control"],
  "sounds": {
...
| 字段                           | 类型     | 必需 | 默认值                | 说明                                                                          |
| ---------------------------- | ------ | -- | ------------------ | --------------------------------------------------------------------------- |
| `steering_center`            | 三维向量   | 否  | `[0, 0, 0]`        | 转向中心点相对于子零件原点的偏移（m）。影响阿克曼转向的几何计算                                            |
...
| `max_drift_angular_velocity` | 浮点数或对象 | 否  | 预设曲线               | 最大漂移角速度与车速的关系。同 `lateral_acceleration`，可以为单值或曲线。控制漂移时的旋转速率                  |
...
| `control_inputs`             | 字符串数组  | 否  | `["move_control"]` | 驾驶控制信号的接收频道名                                                                |
...
| `diff_lock_inputs`         | 字符串数组 | 否  | `["diff_lock_control"]` | 手动差速锁控制信号接收频道名（仅 `"manual"` 模式有效）                                |
...
```json
{
  ...
  "diff_lock_inputs": ["diff_lock_control"]
}
```
...
```json
{
  ...
  "axes": {
    "yr": {
      ...
      "speed_outputs": {"turret_yaw_speed": ["subpart", "vehicle"]},
      "position_outputs": {"turret_yaw_angle": ["subpart", "vehicle"]}
    },
    "xr": {
      "lower_limit": -10.0,
      "upper_limit": 45.0,
...
| 字段                      | 类型    | 必需 | 默认值                                 | 说明                      |
| ----------------------- | ----- | -- | ----------------------------------- | ----------------------- |
| `control_inputs`        | 字符串数组 | 否  | `["wheel_control", "move_control"]` | 车轮控制信号的接收频道名，优先级从高到低    |
...
| 字段                | 类型  | 必需 | 默认值       | 说明                                |
| ----------------- | --- | -- | --------- | --------------------------------- |
| `max_force`       | 浮点数 | 否  | `10000.0` | 最大驱动力（N）。由输入信号值乘以该值得到实际驱动力        |
| `max_brake_force` | 浮点数 | 否  | `3500.0`  | 最大制动力（N）。当无驱动输入时施加的减速力            |
| `max_speed`       | 浮点数 | 否  | `0.0`     | 最大线速度（m/s）。设为 `0` 时不限速。主要配合摩托控制使用 |
| `default_force`   | 浮点数 | 否  | `3140.0`  | 默认驱动力/制动力（N）。当默认状况下无输入时施加         |
...
| `max_speed` | 浮点数 | 否  | `3.14`   | 最大转向角速度（rad/s）。等同于最大转动速率，π rad/s ≈ 180°/s |

```json
{
  "basic_durability": 150.0,
  "control_inputs": ["wheel_control", "move_control"],
  "roll": {
    "max_force": 8000,
  ...
  "main_body": {
    ...
    "subsystems": {
      ...
      "subsystem.my_mod.driver_seat": {
        ...
        "locator": "DriverSeat",
        "move_outputs": {
          "move_control": ["subsystem.my_mod.controller"]
        }
      }
    }
  }
...
| `speed_outputs` | 字符串到字符串数组对象 | 否         | `{"engine_speed": ["vehicle", "subpart"]}` | 转速反馈信号的发送频道与目标。键为频道名，值为目标列表                  |
...
| `gear_outputs` | 字符串到字符串数组对象 | 否  | `{"gear": ["subpart", "vehicle"]}` | 挡位反馈信号的发送频道与目标。输出当前所处的挡位索引供其他系统使用 |
...
| 字段                  | 类型          | 必需 | 默认值                                         | 说明           |
| ------------------- | ----------- | -- | ------------------------------------------- | ------------ |
| `engine_outputs`    | 字符串到字符串数组对象 | 是  | —                                           | 发动机控制信号的发送目标 |
| `wheel_outputs`     | 字符串到字符串数组对象 | 是  | —                                           | 车轮控制信号的发送目标  |
| `gearbox_outputs`   | 字符串到字符串数组对象 | 否  | `{}`                                        | 变速箱控制信号的发送目标 |
| `speed_outputs`     | 字符串到字符串数组对象 | 否  | `{"vehicle_speed": ["subpart", "vehicle"]}` | 车速反馈信号的发送目标  |
| `throttle_outputs`  | 字符串到字符串数组对象 | 否  | `{"throttle": ["subpart", "vehicle"]}`      | 油门反馈信号的发送目标  |
| `steering_outputs`  | 字符串到字符串数组对象 | 否  | `{"steering": ["subpart", "vehicle"]}`      | 转向反馈信号的发送目标  |
| `brake_outputs`     | 字符串到字符串数组对象 | 否  | `{"brake": ["subpart", "vehicle"]}`         | 制动反馈信号的发送目标  |
| `handbrake_outputs` | 字符串到字符串数组对象 | 否  | `{"handbrake": ["subpart", "vehicle"]}`     | 手刹反馈信号的发送目标  |

> 目标列表的写法与连接点 `signal_targets` 相同，可指向子系统名、连接点名、`"subpart"` 或 `"vehicle"`。详见 [5.4 连接点定义 > 信号传输目标](5.4-连接点定义.md)。
...
```json
{
  ...
  "roll_speed_outputs": {
    "wheel_speed_fl": ["subpart", "vehicle"]
  },
  "steering_angle_outputs": {
    "steer_angle_fl": ["subpart", "vehicle"]
  }
}
```
